  ...
  return (
    <div className="py-10 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-12 animate-fade-in">
      ...
      <div className="text-center space-y-3">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#7C3AED]/10 text-[#7C3AED] font-bold text-[9px] uppercase tracking-widest mb-1 border border-[#7C3AED]/20 shadow-sm">
          <Sparkles size={10} />
          <span>Intelligent Audit Engine</span>
        </div>
        
        <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-widest leading-none max-w-4xl mx-auto uppercase">
          Impression <span className="premium-gradient-text font-light lowercase">.pr</span>
        </h1>
      ...
      {error && (
        <div className="max-w-4xl mx-auto bg-red-50 border border-red-200 p-6 rounded-2xl shadow-sm animate-shake">
          <div className="flex gap-4">
            <div className="w-10 h-10 bg-red-100 rounded-xl flex items-center justify-center text-red-600 flex-shrink-0">
               <FileText size={20} />
            </div>
            <div>
              <h3 className="text-sm font-bold text-red-800 uppercase tracking-widest mb-1">Generation Failure</h3>
              <p className="text-xs text-red-600 font-medium opacity-90">{error}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="space-y-10">
         <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-200/60 pb-10">
            ...
            <button 
              ...
              className="flex items-center gap-2 px-6 py-2.5 bg-white border border-slate-200 rounded-full shadow-sm text-sm font-bold text-slate-700 hover:border-[#7C3AED] hover:text-[#7C3AED] transition-all"
            >
         ...
         {loadingHistory ? (
           <div className="py-24 text-center">
              <div className="w-16 h-16 border-4 border-slate-200 border-t-[#7C3AED] rounded-full mx-auto animate-spin mb-6"></div>
              <p className="text-slate-400 font-semibold tracking-widest uppercase text-xs">Syncing Cloud Data...</p>
           </div>
         ) : (history && history.length > 0) ? (
           <div className="glass-card rounded-[2.5rem] overflow-hidden border border-slate-200/50 shadow-xl shadow-purple-900/5">
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {history.map((item, idx) => (
                      <tr key={idx} className="hover:bg-purple-50/50 transition-all group">
                        <td className="px-8 py-7">
                           <span className="text-xs font-bold text-slate-700 bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200/60">{item.client_id}</span>
                        </td>
                        <td className="px-8 py-7">
                        ...
                        {isAdmin && (
                          <td className="px-8 py-6">
                             <div className="flex items-center gap-2">
                                <div className="w-8 h-8 bg-[#7C3AED]/10 rounded-full flex items-center justify-center text-[#7C3AED] font-bold text-xs">
           ...
           <div className="glass-card rounded-[2rem] p-20 text-center">
              ...
              <h3 className="text-2xl font-bold text-slate-800 mb-2">No Audits Found</h3>
